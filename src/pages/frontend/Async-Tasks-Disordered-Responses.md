...
meta:
  - name: description
    content: 从搜索框提示词响应乱序问题出发，说明问题的本质在于响应不是有序到达导致结果被覆盖，介绍三种解决方案——异步任务队列、取消请求和清理函数。
  - name: keywords
    content: 响应乱序,竞态条件,请求取消,清理函数
---
...
在实现中，如果用户每输入一个字符便进行一次提示词请求的发送，会导致一个问题，就是<u>响应不一定是有序到达</u>的，最后一次的响应并不一定对应最后一次发送的请求，网络延迟、数据包丢失或服务器处理速度差异都可能会导致响应无序到达。
