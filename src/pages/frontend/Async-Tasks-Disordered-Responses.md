...
在实现中，如果用户每输入一个字符便进行一次提示词请求的发送，会导致一个问题，就是<u>响应不一定是有序到达</u>的，最后一次的响应并不一定对应最后一次发送的请求。那为什么响应可能会无序到达呢？我们可以从不同 HTTP 版本下的行为来解释：

- **HTTP/1.0**：采用短连接，每个请求都会建立一个新的 TCP 连接。因此，多个请求会建立多个不同的 TCP 连接。由于每个请求的连接是独立的，网络延迟、数据包丢失或服务器处理速度差异可能导致响应无序到达。例如，先发送的请求可能因为连接延迟而后收到响应，而后发送的请求可能先收到响应。
- **HTTP/1.1**：采用持久连接，允许在同一个连接上发送多个请求，且同一连接上的请求可以连续发送，无需等待前一个请求的响应（流水线机制）。理论上，请求和响应是顺序处理的（FIFO），即响应必须按照请求的顺序返回。TCP 协议本身保证数据包按序交付，因此在同一个连接上，响应应该是有序到达的。然而与 HTTP/1.0 的情况相同，客户端通常会对同一域名下的多个请求建立多个连接（浏览器默认限制 6 个并发连接），以提升并发性能。在这种情况下，即使使用 HTTP/1.1，请求可能会被分配到不同连接上，从而导致响应可能仍无序到达。
- **HTTP/2**：2.0 版本下支持了多路复用，允许在同一个连接上并发发送多个请求和响应。请求和响应通过流ID（stream ID）来进行标识，因此服务器可以以任意顺序返回响应，这意味着响应肯定会存在无序到达。如果用户快速输入字符，发送多个请求，先发送的请求可能由于服务器处理延迟而后收到响应，而后发送的请求可能先完成处理并返回。

![20241014130819](./20241014130819.png)

有人就说了，可以通过**防抖**来解决。但一样地，采用防抖仍然存在乱序问题，防抖仅仅只是处理了事件在短时间内多次触发只执行最后一次回调，并不能实际解决响应乱序问题。仍然存在第一次请求的响应由于网路环境因素导致延后到达，使得后续发起请求的响应被这个延后响应的结果覆盖。

## 异步任务队列

那如何解决这个问题呢？我们可以发现这些问题的本质就是由于**响应不是有序到达**所导致的，那么可以在应用层层面实现一个异步任务队列，把请求都加入到队列中，<u>当前一个请求响应到达后，再去发起下一个请求</u>，从而保证**用户操作（请求）的顺序跟响应的顺序一致**。
