...
终于在 Vite 8.0 中，Vite 引入了 `Rolldown` 替换了开发环境下的 `esbuild` 和生产环境下的 `Rollup`，解决了开发环境和生产环境下可能的不一致性问题（不同的转换行为、各自独立的插件系统）。

`Rolldown` 的打包构建速度非常快，这使得 Vite 在未来也在考虑**将完整的打包构建流程引入到开发环境**中，以解决在大型项目中大量未打包的网络请求而导致<u>页面加载缓慢</u>的问题。
