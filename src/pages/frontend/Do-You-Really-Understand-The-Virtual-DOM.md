...
也就是此时视图发生了变化，对于框架本身并不知道哪里发生了改变，因此要如何去更新真实的 DOM 是一个问题。有人说，我直接把原来的 DOM 销毁，根据当前状态重新创建新的 DOM 树不就可以了吗？确实这种方案简单直接，但缺点也很明显，性能极差，涉及到全量的 DOM 销毁和重建，且会丢失所有内部的状态（如输入框焦点、滚动位置等）。因此，这种方案不可能作为声明式框架的最佳实现。

一种方案就是，在运行时找到<u>**前后的差异**并只更新变化的地方</u>，但是最终完成这次更新的代码仍然是：

```js
div.textContent = 'hello vue3' // 直接修改
```

所以我们可以发现，声明式框架的视图更新会比命令式代码多了**找出差异的性能消耗**。因此最理想的情况是，当找出差异的性能消耗为 0 时，声明式代码与命令式代码的性能相同，但是**无法做到超越**，毕竟<u>框架本身就是封装了命令式代码</u>才实现了面向用户的声明式。
...
刚刚我们说完了关于利用虚拟 DOM 实现声明式框架的方案，那么另一种可能的方案则是实现一个**编译器**，这个编译器<u>可以精确地标记 “哪个状态对应哪个 DOM 操作”，状态变化时直接执行该操作，从而完成视图更新</u>。Svelte 就是这么做的，详细可以看这篇[文章](https://juejin.cn/post/6844904100656594957)。

但这种方案极其复杂，想要实现一个这样的编译器并非易事。对于每个状态变量，编译器需要知道它影响了哪些 DOM 节点，以及如何更新这些节点，且要求编译器能够生成最小的、直接的 DOM 操作指令。当模板中存在很多动态部分时，由于条件分支和循环项的数量和顺序可能随着状态的变化而变化，编译器需要生成能够处理动态结构的代码，同时尽可能地复用已有的 DOM 节点。这种试图在**静态**阶段分析**动态**的 JavaScript 程序本身就是极度困难的，这个方案需要极深的编译原理理解和大量的工程实践才能走通。
...
![20241018130059](./20241018130059.png)
