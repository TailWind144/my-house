...
1. 首先搜索<u>浏览器中的 DNS 缓存</u>；
2. 如果没有命中，则搜索<u>操作系统本地的 DNS 缓存</u>；
3. 若仍未命中，则客户端向**本地域名服务器**发起**递归查询**；
4. 本地域名服务器会向上级的域名服务器进行**迭代查询**：

   - 先向**根域名服务器**发起查询请求，根域名服务器会将<u>顶级域名（`.com`）服务器的地址</u>返回给本地域名服务器；

   - 然后向**顶级域名服务器**发起查询请求，顶级域名服务器会将<u>权威域名服务器（`example.com`）的地址</u>返回给本地域名服务器；

   - 最后向**权威域名服务器**发起查询请求，最终得到该域名（`www.example.com`）对应的 IP 地址。
