...
![20240803152233](./20240803152233.png)

首先，在讲 ResNet 的主要思想前，我们来看看上面这两张表，这两张表来源于最初提出 ResNet 的文章当中。这两张表表述的是一个结构相同，但层数不同的常规 CNN 在相同 iter（迭代的总数据量） 下的训练误差和测试误差。可以发现层数更多的网络所得到的误差反而更大，这并不是过拟合的现象，因为训练误差和测试误差都很大。
...
现在我们再回过头来看之前为什么更深的网络会得到更差的误差，其实原因是在于更深的网络如果没有做其他方面的处理，而是使用与浅层网络相同的参数和方法去训练，那么此时的模型就很容易由于后续的梯度太小导致训练不动了，而且也没有收敛到一个很好的结果。而 ResNet 的残差连接使得网络的梯度不会由于层数的增多而导致梯度消失，所以在同样的情况下，ResNet 能够收敛得很好。
